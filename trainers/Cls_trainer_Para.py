...
class ClsTrainer_Para(Trainer):
    def __init__(self, model, args, metrics: Dict[str, Callable], main_metric: str,
        ...
        super(ClsTrainer_Para, self).__init__(model, args, metrics, main_metric, device, tensorboard_functions,
...
#         print(batch[0])
        ...
        # print(batch)
        Ab_ft = torch.from_numpy(amino_seq_to_one_hot(batch['H'][0]+batch['L'][0],'antibody' ,antibody_max_len=self.model.max_antibody_len, virus_max_len=self.model.max_virus_len))
        Ag_ft = torch.from_numpy(amino_seq_to_one_hot(batch['spike'][0],'virus', antibody_max_len=self.model.max_antibody_len, virus_max_len=self.model.max_virus_len))
#         print(Ab_ft.shape)
        cls_pred= self.model(Ab_ft.to(self.device),Ag_ft.to(self.device),torch.tensor([len(batch['H'][0]+batch['L'][0])]),torch.tensor([len(batch['spike'][0])])) # forward the sequence to the model
