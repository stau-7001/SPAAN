...
import json
from commons.losses import CpLoss
...
class CPTrainer(Trainer):
    ...
    def forward_pass(self, batch, precompute=True):
        #         print(batch[0])
        if precompute:
            cp_pred, combined_representation = self.model(batch)
        else:
            cp_pred, combined_representation = self.model(
                batch["H"], batch["L"], batch["spike"]
            )  # forward the sequence to the model

        reg_label = torch.exp(
            torch.FloatTensor(batch["IC50"]).to(self.device).unsqueeze(0)
        )
        #         print(pred.shape, label.shape, label.float().unsqueeze(0).shape)
        loss = self.loss_func(cp_pred, reg_label, self.epoch)
        # print(reg_pred.shape,reg_label.shape)
        # print(reg_pred,reg_label)
        # return loss, reg_pred ,reg_label
        return loss, cp_pred, reg_label, combined_representation
    ...
    def process_batch(self, batch, optim):
        loss, reg_pred, reg_label, combined_representation = self.forward_pass(batch)
            ...
            with torch.no_grad():
                ...
                if self.optim_steps % self.args.log_iterations == 0 and optim != None:
                    if self.optim_steps % self.accumulation_steps == 0:  # predict
                        metrics = self.evaluate_metrics(
                            reg_accumulated_preds, reg_accumulated_targets
                        )
                ...
                if (
                    ...
                    metrics_results = self.evaluate_metrics(
                        reg_accumulated_preds, reg_accumulated_targets, val=True
                    )
        ...
        if optim == None:
            if self.val_per_batch:
                ...
                total_metrics = self.evaluate_metrics(
                    reg_epoch_predictions, reg_epoch_targets, val=True
                )
    ...
    def evaluate_metrics(
        self, reg_predictions, reg_targets, batch=None, val=False
    ) -> Dict[str, float]:
        metrics = {}
    ...
    def evaluation(self, data_loader: DataLoader, data_split: str = ""):
        self.model.eval()
        metrics = self.predict(data_loader, epoch=2, data_split=data_split)

        with open(
            os.path.join(self.writer.log_dir, "evaluation_" + data_split + ".txt"), "w"
        ) as file:
