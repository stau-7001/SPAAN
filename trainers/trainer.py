...
class Trainer():
    def __init__(self, model, args, metrics: Dict[str, Callable], main_metric: str, device: torch.device,
        ...
        if args.checkpoint:
            ...
            # 
            # new_lr_value = 0.00006
            # for param_group in self.optim.param_groups:
            #     param_group['lr'] = new_lr_value
            if self.lr_scheduler != None and checkpoint['scheduler_state_dict'] != None:
                # print('loading scheduler')
                # # print(checkpoint['scheduler_state_dict'])
                # checkpoint['scheduler_state_dict']['_last_lr'] = [0.00006]
                # checkpoint['scheduler_state_dict']['start_lrs'] = [0.00006]
                # checkpoint['scheduler_state_dict']['patience'] = 5
    ...
    def predict(self, data_loader: DataLoader, epoch: int, optim: torch.optim.Optimizer = None,
        ...
        for i, batch in enumerate(data_loader):
            ...
            with torch.no_grad():
                accumulated_preds = torch.cat((predictions, accumulated_preds), 0)
