...
#         print(batch[0])
        if precompute:
            reg_pred,combined_representation = self.model(batch)  # forward the sequence to the model
        else:
            reg_pred,combined_representation = self.model(batch['H'],batch['L'],batch['spike'])  # forward the sequence to the model
...
#         print(pred.shape, label.shape, label.float().unsqueeze(0).shape)
        ...
        return loss, reg_pred ,reg_label, combined_representation
