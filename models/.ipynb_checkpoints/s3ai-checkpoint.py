...
class MultiTask_SPAAN(nn.Module):
    def __init__(
        ...
        self.regression_head = RegressionHead(esm_hidden_dim + str_hidden_dim, 1).to(
            device
        )
    ...
    def forward(self, Abseq_H, Abseq_L, Agseq):
        ...
        for batch_idx in range(att_data.shape[0]):
            ...
            plt.imshow(
                att_data[batch_idx].cpu(), cmap="viridis", aspect="auto", vmin=0, vmax=1
            )
            plt.colorbar()
