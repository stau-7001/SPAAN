...
class ContextPooling(nn.Module):
    ...
    def _local_normal(self, s, center, seq_len, r=0.1):
        ...
        mean_ = center
        place = (
            torch.arange(seq_len).float().repeat(std_.shape[0], 1).to(device)
        )  # [B,L]
        ...
        ret = (
            pow(2 * PI, -0.5)
            * torch.pow(std_, -1)
            * torch.exp(-torch.pow(place - mean_, 2) / (1e-5 + 2 * torch.pow(std_, 2)))
        )
    ...
    def forward(self, feats):  # feats: [B,L,1024]
        ...
        for i in range(feats.shape[-2]):
            ...
            out.append(
                (w_ * feats).sum(1, keepdim=True)
            )  # w_ [B,L,1], feats [B,L,1024]
...
class MultiTask_SPAAN_with_cnn_context(nn.Module):
    def __init__(
        self, device, esm_hidden_dim=640, str_hidden_dim=64, alpha=0.8, **kwargs
    ):
        super(MultiTask_SPAAN_with_cnn_context, self).__init__()
        self.device = device
        ...
        self.classification_head = ClassificationHead(
            esm_hidden_dim + str_hidden_dim, 2
        )
        self.alpha = alpha
...
class MultiTask_SPAAN_nba(nn.Module):
