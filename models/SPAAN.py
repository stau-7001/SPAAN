...
def fixed_attention_weighted_feature(feature, weights, alpha=0.8):
    ...
    normalized_weights = F.softmax(
        alpha * torch.ones_like(weights) + (1 - alpha) * weights
    )
...
class MultiTask_SPAAN(nn.Module):
    """
    ...
    def __init__(
        self, device, esm_hidden_dim=640, str_hidden_dim=64, alpha=0.8, **kwargs
    ):
        super(MultiTask_SPAAN, self).__init__()
        self.device = device
    ...
    def forward(self, Abseq_H, Abseq_L, Agseq):
        Zab, Ab_batch_lens = self.Ab_encoder(
            Abseq_H, Abseq_L
        )  # shape: (batch_size, Abseq_len, esm_hidden_dim=640), (batch_size,)
        Zag, Ag_batch_lens = self.Ag_encoder(
            Agseq, [""] * len(Agseq)
        )  # shape: (batch_size, Agseq_len, esm_hidden_dim=640), (batch_size,)
        #         Zag = fixed_attention_weighted_feature(Zag, *weight,self.alpha)
        # print(Zag.device)
        att_data, Att_ab_ag = self.Bi_Att(
            Zab, Zag, True
        )  # shape: (batch_size, Abseq_len, esm_hidden_dim=640)
        ...
        for i, tokens_len in enumerate(Ab_batch_lens):
            sequence_representations_int.append(
                Att_ab_ag[i, 1 : tokens_len - 1].mean(0)
            )  # [esm_hidden_dim]

        x_int = torch.stack(
            sequence_representations_int, dim=0
        )  # shape: (batch_size, esm_hidden_dim)

        sequence_representations_str = []
        for i, tokens_len in enumerate(Ab_batch_lens):
            sequence_representations_str.append(
                Str_Zab[i, 1 : tokens_len - 1].mean(0)
            )  # [str_hidden_dim]

        x_str = torch.stack(
            sequence_representations_str, dim=0
        )  # shape: (batch_size, str_hidden_dim)

        x_reg_final = self.regression_head(
            torch.cat((x_str, x_int), dim=1)
        )  # shape: (batch_size, 1)
        x_cls_final = self.classification_head(
            torch.cat((x_str, x_int), dim=1)
        )  # shape: (batch_size, 1)
...
class CNNModule(nn.Module):
    def __init__(self, max_Agseq_len_rbd, max_Abseq_len, esm_hidden_dim):
        ...
        self.conv1 = nn.Conv2d(
            in_channels=esm_hidden_dim,
            out_channels=esm_hidden_dim * 2,
            kernel_size=3,
            padding=1,
        )
        self.conv2 = nn.Conv2d(
            in_channels=esm_hidden_dim * 2,
            out_channels=esm_hidden_dim * 2,
            kernel_size=3,
            padding=1,
        )
        ...
        # self.bn0 = nn.LayerNorm(esm_hidden_dim)
        # self.bn1 = nn.LayerNorm(esm_hidden_dim * 2)
        # self.bn2 = nn.LayerNorm(esm_hidden_dim * 2)
        # self.bn3 = nn.LayerNorm(esm_hidden_dim)
    ...
    def forward(self, x):
        ...
        # self.activations["x"] = x
        # self.activations["input"] = x
        # x = self.bn0(x).permute(0, 3, 1, 2)
        # self.activations["input_bn"] = x
        # self.activations["conv1"] = self.bn1(self.elu(self.conv1(x)).permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        # x = self.activations["conv1"]
        # self.activations["conv2"] = self.bn2(self.elu(self.conv2(x)).permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        # x = self.activations["conv2"]
        # self.activations["conv3"] = self.bn3(self.elu(self.conv3(x)).permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        # x = self.activations["conv3"]
...
class CNNModule_light(nn.Module):
    def __init__(self, max_Agseq_len_rbd, max_Abseq_len, input_hidden_dim=2):
        ...
        # Define your CNN layers here
        self.conv1 = nn.Conv2d(
            in_channels=input_hidden_dim, out_channels=64, kernel_size=3, padding=1
        )
        self.conv2 = nn.Conv2d(
...
class MultiTask_SPAAN_INTER(nn.Module):
    def __init__(
        self,
        device,
        esm_hidden_dim=640,
        ...
        **kwargs
        ...
        super(MultiTask_SPAAN_INTER, self).__init__()
        self.device = device
        ...
        self.conv_module = CNNModule(
            self.max_Ag_len, self.max_Ab_len, esm_hidden_dim
        ).to(device)
    ...
    def forward(self, Abseq_H, Abseq_L, Agseq):
        ...
        inter_batch = []
        for i, tokens_len in enumerate(Ab_batch_lens):
            Zag_inter = Zag[i, RBD_start : RBD_end + 1].unsqueeze(
                1
            )  # [batch_size, Agseq_len_rbd, 1 esm_hidden_dim]
            Zab_inter = Zab[i, 1 : tokens_len - 1].unsqueeze(
                0
            )  # [batch_size, 1, Abseq_len, esm_hidden_dim]
            ...
            inter_batch.append(
                F.pad(
                    inter_matrix.unsqueeze(0),
                    (0, 0, 1, l2_padding - 1, 1, l1_padding - 1),
                    "constant",
                    0,
                ).squeeze(0)
            )
        x_inter = torch.stack(
            inter_batch, dim=0
        )  # [batch_size, max_Agseq_len_rbd, max_Abseq_len, esm_hidden_dim]
        # print(x_inter.shape)
...
class MultiTask_SPAAN_INTER_Hbond(nn.Module):
    def __init__(
        self,
        device,
        esm_hidden_dim=640,
        ...
        **kwargs
        ...
        super(MultiTask_SPAAN_INTER_Hbond, self).__init__()
        self.device = device
        self.max_Ab_len = max_Ab_len
        self.max_Ag_len = max_Ag_len
        ...
        self.conv_module = CNNModule(
            self.max_Ag_len, self.max_Ab_len, esm_hidden_dim + 2
        ).to(device)
    ...
    def forward(self, Abseq_H, Abseq_L, Agseq):
        ...
        for i, tokens_len in enumerate(Ab_batch_lens):
            ...
            Zag_inter = Zag[i, RBD_start : RBD_end + 1].unsqueeze(
                1
            )  # [Agseq_len_rbd, 1 esm_hidden_dim]
            Zab_inter = Zab[i, 1 : tokens_len - 1].unsqueeze(
                0
            )  # [ 1, Abseq_len, esm_hidden_dim]
            # print(Zab_inter.shape,Zag_inter.shape)
            inter_matrix = Zag_inter * Zab_inter
            # print(inter_matrix.shape)
            Hbond_matrix = generate_Hbond_tensor(
                Abseq_H[i] + Abseq_L[i], Agseq[i][RBD_start : RBD_end + 1]
            ).to(self.device)
...
class MultiTask_SPAAN_Chem(nn.Module):
    def __init__(
        self,
        device,
        esm_hidden_dim=640,
        ...
        **kwargs
        ...
        super(MultiTask_SPAAN_Chem, self).__init__()
        self.device = device
        self.max_Ab_len = max_Ab_len
        self.max_Ag_len = max_Ag_len
        self.virus_name = virus_name
        ...
        self.conv_module = CNNModule(
            self.max_Ag_len, self.max_Ab_len, esm_hidden_dim + 5
        ).to(device)
    ...
    def forward(self, Abseq_H, Abseq_L, Agseq):
        with torch.no_grad():
            Zab, Ab_batch_lens = self.Ab_encoder(Abseq_H, Abseq_L)
            Zag, Ag_batch_lens = self.Ag_encoder(Agseq, [""] * len(Agseq))
        ...
        for i, tokens_len in enumerate(Ab_batch_lens):
            ...
            inter_matrix = Zag_inter * Zab_inter
            # print(inter_matrix.shape)
            Chem_matrix = generate_Chem_tensor(
                Abseq_H[i] + Abseq_L[i], Agseq[i][RBD_start : RBD_end + 1]
            ).to(self.device)
...
class MultiTask_SPAAN_Chem_pre(nn.Module):
    def __init__(
        self,
        device,
        esm_hidden_dim=640,
        ...
        **kwargs
        ...
        super(MultiTask_SPAAN_Chem_pre, self).__init__()
        self.device = device
        self.max_Ab_len = max_Ab_len
        self.max_Ag_len = max_Ag_len
        self.virus_name = virus_name
    ...
    def forward(self, batch):
        H_embeddings = [
            embedding[1 : length - 1, :].to(self.device)  # 保留 hidden_dim 维度
            for embedding, length in zip(batch["H_embedding"], batch["H_length"])
        ]
        L_embeddings = [
            embedding[1 : length - 1, :].to(self.device)  # 保留 hidden_dim 维度
            for embedding, length in zip(batch["L_embedding"], batch["L_length"])
        ]
        Ag_embeddings = [
            embedding[1 : length - 1, :].to(self.device)  # 保留 hidden_dim 维度
            for embedding, length in zip(batch["Ag_embedding"], batch["Ag_length"])
        ]

        HL_embeddings = [
            torch.cat([h, l], dim=0)  # 按 seq_len 维度拼接
            for h, l in zip(H_embeddings, L_embeddings)
        ]

        Zab = torch.stack(
            HL_embeddings, dim=0
        )  # shape: (batch_size, seq_len, esm_hidden_dim)
        Zag = torch.stack(
            Ag_embeddings, dim=0
        )  # shape: (batch_size, seq_len, esm_hidden_dim)
        ...
        Ab_batch_lens = batch["L_length"] + batch["H_length"]
        Ag_batch_lens = batch["Ag_length"]
        ...
        for i, tokens_len in enumerate(Ab_batch_lens):
            ...
            Zab_inter = Zab  # [ 1, Abseq_len, esm_hidden_dim]
            # print(Zab_inter.shape,Zag_inter.shape)
            inter_matrix = Zag_inter * Zab_inter
            # print(inter_matrix.shape)
            Chem_matrix = generate_Chem_tensor(
            ...
            # inter_batch.append(inter_matrix)
        x_inter = torch.stack(
            inter_batch, dim=0
        )  # [batch_size, max_Agseq_len_rbd, max_Abseq_len, esm_hidden_dim]
        # print(x_inter.shape)
        ...
        combined_representation = torch.cat((x_str, x_inter), dim=1)
        # combined_representation = x_inter
...
class MultiTask_SPAAN_Chem_HIV(nn.Module):
    def __init__(
        self,
        device,
        esm_hidden_dim=640,
        ...
        **kwargs
        ...
        super(MultiTask_SPAAN_Chem_HIV, self).__init__()
        self.device = device
        self.max_Ab_len = max_Ab_len
        self.max_Ag_len = max_Ag_len
        self.virus_name = virus_name
    ...
    def forward(self, Abseq_H, Abseq_L, Agseq):
        ...
        for i, tokens_len in enumerate(Ab_batch_lens):
            ...
            inter_matrix = Zag_inter * Zab_inter
            Chem_matrix = generate_Chem_tensor(
                Abseq_H[i].replace("X", "G") + Abseq_L[i].replace("X", "G"),
                Agseq[i][RBD_start : RBD_end + 1].replace("X", "G"),
            ).to(self.device)
            inter_matrix = torch.cat([inter_matrix, Chem_matrix], dim=2)
...
class Cls_SPAAN_Chem(nn.Module):
    def __init__(
        self,
        device,
        esm_hidden_dim=640,
        ...
        **kwargs
        ...
        super(Cls_SPAAN_Chem, self).__init__()
        self.device = device
        self.max_Ab_len = max_Ab_len
        self.max_Ag_len = max_Ag_len
        self.virus_name = virus_name
    ...
    def random_crop(self, tensor, max_crop=10):
        """
        Randomly crop the tensor along the first two dimensions (H, W) using 4 random values (x1, x2, x3, x4),
        where each value is less than max_crop. The third dimension (C) remains unchanged.

        Args:
            tensor (torch.Tensor): Input tensor of shape (H, W, C).
            max_crop (int): Maximum value for the random crop offsets (x1, x2, x3, x4).

        Returns:
            torch.Tensor: Cropped tensor of shape (H - x1 - x2, W - x3 - x4, C).
        """
        import random
        ...
        # Generate 4 random crop offsets (x1, x2, x3, x4) less than max_crop
        x1, x2, x3, x4 = [random.randint(0, max_crop - 1) for _ in range(4)]

        # Calculate crop boundaries
        h_start, h_end = x1, H - x2
        w_start, w_end = x3, W - x4

        # Ensure valid crop boundaries (avoid h_start >= h_end or w_start >= w_end)
        # if h_start >= h_end:
        #     h_start, h_end = 0, H
        # if w_start >= w_end:
        #     w_start, w_end = 0, W
    ...
    def forward(self, batch):
        ...
        for i, tokens_len in enumerate(Ab_batch_lens):
            if self.virus_name == "HIV":
                RBD_start = max(Agseq[i].find("GVP") - 8, 0)
                RBD_end = max(Agseq[i].find("APT") + 15, Agseq[i].find("YKVV") + 25)
            else:
                RBD_start = Agseq[i].find("NITN")
                RBD_end = Agseq[i].find("KKST") + 4
            ...
            Zab_inter = Zab[i, :].unsqueeze(0)  # [ 1, Abseq_len, esm_hidden_dim]
            # print(Zab_inter.shape,Zag_inter.shape)
            inter_matrix = Zag_inter * Zab_inter
            # print(inter_matrix.shape)
            Chem_matrix = generate_Chem_tensor(
            ...
            # inter_matrix = torch.cat([inter_matrix,Hbond_matrix,hydropathy_matrix], dim=2)
            inter_matrix = torch.cat([inter_matrix, Chem_matrix], dim=2)
            # inter_matrix = self.random_crop(inter_matrix)
            # print(inter_matrix.shape)
...
class MultiTask_SPAAN_Escape_pre(nn.Module):
    def __init__(
        self,
        device,
        esm_hidden_dim=640,
        ...
        **kwargs
        ...
        super(MultiTask_SPAAN_Escape_pre, self).__init__()
        self.device = device
        self.max_Ab_len = max_Ab_len
        self.max_Ag_len = max_Ag_len
        self.virus_name = virus_name
    ...
    def feature2out(self, Abseq_H, Abseq_L, activation, target_layer, output_type):
        ...
        x = torch.cat((x_str, x_inter), dim=1)
        if output_type == "classification":
            x_cls_final = self.escape_head(x)  # shape: (batch_size, 1)
            re = x_cls_final[0, 0]
        return re
    ...
    def forward(self, batch):
        ...
        x_es_final = self.escape_head(combined_representation)  # shape: (batch_size, 1)

        return x_es_final, combined_representation


class Reg_SPAAN_Chem(nn.Module):
    def __init__(
        self,
        device,
        esm_hidden_dim=640,
        ...
        **kwargs
        ...
        super(Reg_SPAAN_Chem, self).__init__()
        self.device = device
        self.max_Ab_len = max_Ab_len
        self.max_Ag_len = max_Ag_len
        self.virus_name = virus_name
...
class MultiTask_SPAAN_non_inter(nn.Module):
    def __init__(
        self,
        device,
        esm_hidden_dim=640,
        ...
        **kwargs
        ...
        super(MultiTask_SPAAN_non_inter, self).__init__()
        self.device = device
        self.max_Ab_len = max_Ab_len
        self.max_Ag_len = max_Ag_len
        self.virus_name = virus_name
    ...
    def forward(self, Abseq_H, Abseq_L, Agseq):
        ...
        pooled_Zab = [
            Zab[i, : Ab_batch_lens[i]].mean(0) for i in range(len(Ab_batch_lens))
        ]
        pooled_Zag = [
            Zag[i, : Ag_batch_lens[i]].mean(0) for i in range(len(Ag_batch_lens))
        ]
        # print(x_inter.shape)
        pooled_Str_Zab = [
            Str_Zab[i, 1 : tokens_len - 1].mean(0)
        ...
        x_str_ab = torch.stack(
            pooled_Str_Zab, dim=0
        )  # shape: (batch_size, str_hidden_dim)
        # Combining pooled representations
        combined_representation = torch.cat(
            (x_str, x_ag, x_str_ab), dim=1
        )  # shape: (batch_size, esm_hidden_dim * 2 + str_hidden_dim)
